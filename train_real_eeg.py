...
from dataset.data import Real_EEG
...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='/local_storage/datasets/nonar/ebg/ebg4')
...
if __name__ == "__main__":
    ...
    data_array, labels_array, t, sfreq = utils.load_real_eeg(
        root=data_path,
        subject_id=subject,
    ...
    data = Real_EEG(
        ...
        tmin=tmin, tmax=tmax,
        data_type="sensor_ica", modality=modality,
        pick_subjects=subject, fs_new=fs_new,
    ...
    for i, (train_all_index, test_index) in enumerate(outer_cv.split(data.data, data.labels)):
        for j, (train_index, val_index) in enumerate(
                inner_cv.split(data.data[train_all_index], data.labels[train_all_index])):
            ...
            model = utils.load_model(model_name=model_name, n_channels=ch, n_samples=n_time_samples, n_classes=2)
